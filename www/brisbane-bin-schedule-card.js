class BrisbaneBinScheduleCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const date = next?.state && next.state !== "unavailable" ? next.state : "Unavailable";
    ...
    </style><article><header><h2>${date}</h2><p>${missing ? `Check entity IDs: ${this.config.next_entity} and ${this.config.bins_entity}` : `Collection day: ${collectionDay}`}</p></header><section>${image ? `<img src="${image}" alt="Bins for collection">` : "<p class=\"empty\">No bin image available.</p>"}</section></article>`;
  }
